...
import { TachesBoService } from "./taches-bo.service";
...
@Component({
  selector: "app-taches-bo",
  templateUrl: "./taches-bo.component.html",
  styleUrls: ["./taches-bo.component.scss"],
  animations: [
...
export class TachesBOComponent implements OnInit {
  nameTab = [];
  tab;
  colabMission: string;
  id: number;
        ...
        .subscribe(event => {
          if (event.type === HttpEventType.UploadProgress) {
            ...
            console.log(
              ...
              JSON.parse(event.body).fileDownloadUri.substr(35)
            );
  ...
  getTacheMissionByUser() {
    ...
    this.gestionCollaborateurService.getUserByPseudo(pseudo).subscribe(data => {
      this.id = data.id;
  ...
  updateTache(tache: TacheMission, date) {
    console.log(date, "date bc:kjcb!<");
    console.log(
      this.ELEMENT_DATA,
      "tabbbbbb",
      typeof this.ELEMENT_DATA,
      "type",
      this.ELEMENT_DATA[0],
      "index",
      this.ELEMENT_DATA.indexOf(tache)
    );
  ...
  findTachesUtilisateur() {
    ...
    this.gestionCollaborateurService.getUserByPseudo(pseudo).subscribe(data => {
      this.id = data.id;
      this.tachesUtilisateurService
        .findTachesUtilisateur(this.id)
        .subscribe(data => {
          this.taches = data;
        });
    });
  ...
  colabMissionNames(): any {
    for (let i = 0; i < this.tab.length; i++) {
      this.tachesBoService
        .getColaborateurOfMission(this.tab[i])
        .subscribe(data => {
          const mission: any = data;
          this.nameTab.push(
            mission.collaborateur.nom + " " + mission.collaborateur.prenom
          );
        });
    }
  }
